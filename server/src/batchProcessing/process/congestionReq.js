...
const makeRequest = async (stationCode, dow, hh) => {
  ...
  try {
    ...
    const filePath = path.join(__dirname, "..", "data", "input", "apiResponses", `혼잡_${stationCodes[stationCode]}_${dow}_${hh}.json`);
    fs.writeFileSync(filePath, JSON.stringify(response.data, null, 2));

    const { code, message } = response.data.status;
    console.log(`응답코드: ${code}, 메시지: ${message}`);
    console.log(`역명: ${stationCodes[stationCode]}, 요일: ${dow}, 시간: ${hh} 혼잡 데이터 진행중`);
  } catch (err) {
